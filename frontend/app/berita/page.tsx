...
async function getNews(category?: string, page: number = 1) {
  ...
  if (!res.ok) {
    throw new Error(`HTTP Error Daftar Koleksi: ${res.status}`);
  }
...
export default async function DaftarBerita({
  ...
  const getPageTitle = () => {
    if (!category) return "Koleksi Literasi & Artikel";
    const categoryName = category.charAt(0).toUpperCase() + category.slice(1);
    return `Koleksi ${categoryName}`;
  };

  return (
    <main className="min-h-screen bg-gray-50 flex flex-col">
      <section className="bg-brand-green pt-40 pb-24 px-6 text-center text-white relative overflow-hidden flex flex-col items-center justify-center min-h-[50vh]">
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          ...
          <p className="text-lg md:text-xl text-gray-100 max-w-2xl mx-auto font-light leading-relaxed">
            {category 
              ? `Kumpulan referensi dan tulisan seputar ${category} di Perpustakaan Mu'allimin.` 
              : "Jelajahi berbagai artikel, resensi buku, dan informasi literasi dari Perpustakaan Mu'allimin."}
          </p>
        </div>
      </section>
      ...
      <section className="px-6 pb-24 w-full">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="relative -mt-10 z-20 mb-16 max-w-3xl mx-auto px-4 sm:px-0">
            <div className="bg-white px-6 py-4 md:px-8 md:py-5 rounded-3xl shadow-2xl shadow-green-900/10 border border-gray-100 flex flex-col items-center">
              <div className="w-full">
                <SearchBar />
              </div>
              <div className="mt-3 flex items-center gap-3 text-brand-green font-extrabold tracking-widest uppercase text-xs md:text-sm opacity-90">
          ...
          {paginatedNews.length === 0 && !query && (
             <div className="text-center py-24 bg-white rounded-3xl border border-gray-100 shadow-sm">
               ...
               <p className="text-gray-500 text-lg">Saat ini belum ada tulisan untuk kategori ini.</p>
             </div>
          )}

          {paginatedNews.length === 0 && query && (
            <div className="text-center py-24 bg-white rounded-3xl border border-gray-100 shadow-sm">
              <h3 className="text-2xl font-bold text-brand-green mb-3">Pencarian Tidak Ditemukan</h3>
              <p className="text-gray-500 mb-8 text-lg">Kami tidak menemukan artikel dengan kata kunci &quot;<span className="font-semibold text-brand-green">{query}</span>&quot;.</p>
              <Link href={category ? `/berita?category=${category}` : "/berita"} className="inline-block bg-brand-yellow text-brand-green px-8 py-3.5 rounded-full font-bold hover:bg-yellow-400 transition-all shadow-md hover:-translate-y-1">
                Reset Pencarian
              </Link>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {paginatedNews.map((item: NewsItem) => (
                <Link href={`/berita/${item.slug}`} key={item.id} className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group border border-gray-100 flex flex-col cursor-pointer relative transform hover:-translate-y-1">
                  ...
                  <div className="relative w-full h-60 bg-brand-green flex items-center justify-center overflow-hidden">
                    <span className="absolute z-0 text-brand-yellow font-bold opacity-30 text-2xl px-4 text-center tracking-widest uppercase">Perpustakaan</span>
                    {item.image && (
                      <Image 
                  ...
                  <div className="p-6 flex flex-col grow">
                    ...
                    <h3 className="text-xl font-bold text-brand-green mb-4 line-clamp-2 leading-snug group-hover:text-green-700 transition-colors">
                      {decodeHTMLEntities(item.title)}
                    </h3>
                    <div className="mt-auto pt-5 border-t border-gray-50 flex items-center justify-between">
                      <span className="text-sm font-bold text-brand-green group-hover:text-brand-yellow transition-colors flex items-center gap-2">
                        Baca Selengkapnya
          ...
          {totalPages > 1 && (
            <div className="flex flex-col sm:flex-row justify-center items-center gap-4 sm:gap-8 mt-20">
              ...
              {page > 1 ? (
                <Link href={createPageURL(page - 1)} className="px-8 py-3.5 border-2 border-brand-green text-brand-green rounded-full font-bold hover:bg-brand-green hover:text-white transition-all shadow-sm">
                  &larr; Sebelumnya
                </Link>
              ) : (
              ...
              <span className="font-semibold text-gray-500 bg-white px-8 py-3.5 rounded-full shadow-sm border border-gray-100">
                Halaman <span className="text-brand-green font-bold">{page}</span> dari {totalPages}
              </span>
