...
export default function StaffSection({ staffs, title = "Pustakawan & Staf", showFilters = true }: StaffSectionProps) {
  ...
  const tabs = ['Semua', 'Kepala Perpustakaan', 'Pustakawan', 'Staf Layanan & IT', 'Tenaga Administrasi'];
  ...
  return (
    <section className="py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <h2 className="text-4xl font-extrabold text-brand-green mb-4">{title}</h2>
          <div className="w-24 h-1.5 bg-brand-yellow mx-auto rounded-full mb-6"></div>
          <p className="text-gray-600 max-w-2xl mx-auto text-lg">
            Mengenal lebih dekat para pustakawan dan staf pengelola yang berdedikasi memberikan layanan prima bagi seluruh pemustaka di Perpustakaan Mu&apos;allimin.
          </p>
        </motion.div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
                ...
                <button
                  ...
                  className={`whitespace-nowrap px-6 py-2.5 rounded-lg text-sm font-bold transition-all duration-300 ${
                    activeTab === tab 
                      ...
                      : 'text-gray-500 hover:text-brand-green hover:bg-green-50'
                  }`}
                >
                  {tab}
                </button>
            ...
            <div className="relative w-full md:w-72">
              <input
                ...
                className="w-full pl-11 pr-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand-green focus:border-transparent transition-shadow text-sm"
              />
            ...
            {filteredStaffs.map((staff, index) => (
              ...
              >
                <div className="relative h-72 w-full bg-gray-100 overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-linear-to-t from-brand-green/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-6">
                    <div className="text-white text-center transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 px-4">
                       <p className="text-xs font-light uppercase tracking-widest mb-1 opacity-80">Perpustakaan Mu&apos;allimin</p>
                       <p className="text-sm font-bold">{staff.subject || 'Staf Layanan'}</p>
                    </div>
                  </div>
                </div>
        ...
        {filteredStaffs.length === 0 && (
          ...
          >
            ...
            <p className="text-gray-500">Coba gunakan kata kunci lain untuk mencari pustakawan atau staf.</p>
          </motion.div>
        )}
      </div>
    </section>
