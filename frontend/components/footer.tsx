...
export default function Footer({ settings }: FooterProps) {
  ...
  // Mengatasi data dari API yang berbentuk Array
  ...
  return (
    <footer className="bg-brand-blue text-white pt-16 pb-8 border-t-4 border-brand-yellow">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          ...
          <div>
            {currentSetting?.footer_logo ? (
              <div className="mb-6 relative w-20 h-20 bg-white rounded-2xl p-2 shadow-lg">
                <Image 
                  // [PERBAIKAN]: Menghapus tulisan '/storage' yang dobel
              ...
              <div className="w-16 h-16 bg-white text-brand-blue font-bold flex items-center justify-center rounded-2xl mb-6 shadow-lg">
                LOGO
              </div>
            )}
            <h3 className="text-2xl font-bold mb-3 text-brand-yellow">
              {currentSetting?.site_name || "Madrasah Mu'allimin"}
            </h3>
            <p className="text-gray-300 leading-relaxed text-sm">
              Mencetak kader ulama, pendidik, dan pemimpin bangsa yang berkemajuan sejak tahun 1918.
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-gray-300">
              <li>
                <Link href="/profil" className="hover:text-brand-yellow hover:translate-x-1 transition-all inline-block">
                  Profil & Sejarah
                </Link>
              </li>
              <li>
              ...
              <li>
                <Link href="#" className="hover:text-brand-yellow hover:translate-x-1 transition-all inline-block">
                  Portal PPDB
                </Link>
              </li>
            </ul>
          ...
          <div>
            ...
            <div className="space-y-4 text-gray-300 text-sm">
              ...
              <p className="flex items-center gap-3">
                ...
                {currentSetting?.email || "info@muallimin.sch.id"}
              </p>
              <p className="flex items-center gap-3">
            ...
            {(currentSetting?.instagram || currentSetting?.youtube) && (
              <div className="flex gap-4 mt-6">
                {currentSetting.instagram && (
                  <a href={currentSetting.instagram} target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-blue-800 rounded-full flex items-center justify-center hover:bg-brand-yellow hover:text-brand-blue transition-colors">
                ...
                {currentSetting.youtube && (
                  <a href={currentSetting.youtube} target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-blue-800 rounded-full flex items-center justify-center hover:bg-brand-yellow hover:text-brand-blue transition-colors">
        ...
        <div className="border-t border-blue-800/50 pt-8 text-center text-sm text-gray-400">
          &copy; {new Date().getFullYear()} {currentSetting?.site_name || "Madrasah Mu'allimin Muhammadiyah Yogyakarta"}.
        </div>
      </div>
    </footer>
  );
