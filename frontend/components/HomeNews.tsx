    ...
    <section className="py-20 px-6 w-full bg-gray-50">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12">
          <div>
            <h2 className="text-3xl font-bold text-brand-blue mb-2">Berita & Artikel Terkini</h2>
            <div className="w-24 h-1.5 bg-brand-yellow rounded-full"></div>
          </div>
          <Link href="/berita" className="text-brand-blue font-bold hover:text-brand-yellow transition-colors hidden md:block mt-6 md:mt-0">
          ...
          {news && news.length > 0 ? (
            news.slice(0, 3).map((item) => (
              <Link href={`/berita/${item.slug}`} key={item.id} className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group border border-gray-100 flex flex-col cursor-pointer relative">
                ...
                <div className="relative w-full h-56 bg-brand-blue flex items-center justify-center overflow-hidden">
                  <span className="absolute z-0 text-brand-yellow font-bold opacity-40 text-2xl px-4 text-center tracking-widest uppercase">
                    Mu&apos;allimin
                  </span>
                  {item.image && (
                    <Image 
                ...
                <div className="p-6 flex flex-col grow">
                  ...
                  <h3 className="text-lg font-bold text-brand-blue mb-3 line-clamp-2 leading-snug group-hover:text-blue-700 transition-colors">
                    {decodeHTMLEntities(item.title)}
                  </h3>
                  <div className="mt-auto pt-4 border-t border-gray-100 flex justify-between items-center">
                    <span className="text-sm font-semibold text-brand-blue group-hover:text-brand-yellow transition-colors">
             ...
             <p className="col-span-full text-center text-gray-400">Belum ada berita diterbitkan.</p>
          )}
        </div>
        
        <div className="mt-8 text-center md:hidden">
            <Link href="/berita" className="inline-block border-2 border-brand-blue text-brand-blue px-6 py-2 rounded-full font-bold hover:bg-brand-blue hover:text-white transition-colors">
              Lihat Semua Berita
            </Link>
        </div>
      </div>
