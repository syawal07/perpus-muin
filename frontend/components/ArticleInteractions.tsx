...
export default function ArticleInteractions({ slug, initialLikes, title }: ArticleInteractionsProps) {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      await fetch(`${apiUrl}/api/posts/${slug}/like`, { 
        method: 'POST',
  ...
  return (
    <div className="flex gap-4">
      ...
      >
        ...
        <span className="font-bold">{likes} Suka</span>
      </button>

      <button 
        onClick={handleShare}
        className="flex items-center gap-2 bg-[#25D366] text-white p-3 px-5 rounded-full hover:bg-[#128C7E] transition-all shadow-sm border border-transparent"
      >
        ...
        <span className="font-bold hidden sm:block">Bagikan ke WA</span>
      </button>
    </div>
  );
}