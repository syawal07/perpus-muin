    ...
    <section className="py-20 px-6 w-full bg-white border-t border-b border-gray-100">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12">
          <div>
            <h2 className="text-3xl font-bold text-brand-green mb-3">Agenda & Kegiatan</h2>
            <div className="w-24 h-1.5 bg-brand-yellow rounded-full"></div>
          </div>
          <Link href="/agenda" className="text-brand-green font-bold hover:text-brand-yellow transition-colors hidden md:block mt-6 md:mt-0">
          ...
          {agendas.length > 0 ? (
            agendas.slice(0, 3).map((agenda) => (
              <Link href={`/agenda/${agenda.slug}`} key={agenda.id} className="flex gap-6 items-start p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow bg-white cursor-pointer group">
                <div className="bg-brand-green text-white rounded-xl p-4 text-center min-w-20 shrink-0 shadow-inner group-hover:bg-green-800 transition-colors">
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-800 mb-2 leading-snug group-hover:text-brand-green transition-colors line-clamp-2">
        ...
        <div className="mt-8 text-center md:hidden">
            <Link href="/agenda" className="inline-block border-2 border-brand-green text-brand-green px-6 py-2 rounded-full font-bold hover:bg-brand-green hover:text-white transition-colors">
              Lihat Semua Agenda
            </Link>
        </div>
      </div>
